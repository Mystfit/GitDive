...
FormatterPtr LineFormatterManager::getFormatter(const std::string &elem) const{
    if (!formatterMap[elem].get()) {
        formatterMap[elem] = boost::shared_ptr<LineFormatter>(new LineFormatter(m_lineElementManger, elem, m_targetLine));
    }
...
void LineFormatter::format(const std::string &s, const srchilite::FormatterParams *params) {
    ...
    //Get a free element from the elemManager and let it handle creating new elements for us (I swear this is such a bloody hack)
