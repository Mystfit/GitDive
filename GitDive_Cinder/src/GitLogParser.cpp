    ...
    for(int i = 0; i < diffBlock.size(); i++){
        ...
        if(starts_with(line, "diff --git")){
            ...
            if(starts_with(line, "@@ "))
            {
                ...
                diffHunk->setPostChangedLines(<#int linePos#>, <#int numLines#>)(atoi( Utils::checkStrIndexInRange(splitLine, 1, true ).c_str() ), atoi( Utils::checkStrIndexInRange(splitLine, 2, true ).c_str() ));
                inDiffHeader = false;
            }
