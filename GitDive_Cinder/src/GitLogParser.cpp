    ...
    for(int i = 0; i < diffBlock.size(); i++){
        ...
        //Already in an existing hunk, add current line
            ...
            if(!inDiffHeader){
                ...
                if(currentLine.getLineState() == Line::LINE_ADDED || currentLine.getLineState() == Line::LINE_NORMAL) diffHunk->incrementLinePos();
    ...
    for(int i = 0; i < commit.getNumDiffs(); i++){
        ...
        for(int j = 0; j < diff->getNumHunks(); j++){
            ...
            for(int k = 0; k < diffHunk->getNumLines(); k++){
                ...
                else if(strLine.getLineState() == Line::LINE_DELETED) commitText << "|| -" << diffHunk->getLine(k).getStr() << endl;
            }
