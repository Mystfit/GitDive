...
void GitFileManager::applyDiffToFile(GitFile &file, boost::shared_ptr<Diff> diff, bool useSyntaxHighlighting){
    ...
    boost::shared_ptr<LineFormatterManager> formatterManager(new LineFormatterManager(new LineFormatter("", boost::shared_ptr<Line>()), m_tRender->getLineManager() ));

    if(useSyntaxHighlighting) LineFormatter::syntaxParseLines(newLines, LineFormatter::getFileLangType(file.getFilename()), *(formatterManager));
    file.setLines(newLines);
    file.resetLineOrder();
}
...
void GitFileManager::syntaxParseAllFiles(){
    ...
    for(vector< boost::shared_ptr<GitFile> >::iterator it = m_fileList.begin(); it != m_fileList.end(); ++it){
        LineFormatter::syntaxParseLines(it->get()->getLines(), LineFormatter::getFileLangType(it->get()->getFilename()) );
    }
}
